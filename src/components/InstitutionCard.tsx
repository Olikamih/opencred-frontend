import { Building2, Wallet, Briefcase, Check, Plus } from "lucide-react";
...
export function InstitutionCard({ inst, onConnect }: { inst: Institution; onConnect?: (i: Institution) => void }) {
  ...
  return (
    <div className="glass group relative overflow-hidden rounded-2xl p-5 transition hover:border-white/20">
      ...
      <button
        ...
        className="relative mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm font-medium transition hover:bg-white/10 disabled:opacity-50"
      >
        {inst.connected ? (
          <>Reconectar</>
        ) : (
